...
bool MComplex<C,BT>::markPair_Red(Cell<C,BT>* queen, Cell<C,BT>* king, deque<Cell<C,BT>*>& Queue, num rootdim,
                    typename CLIST::iterator& qpos, typename CLIST::iterator& kpos, bool makekgens)
{
...
bool MComplex<C,BT>::markPair_Cored(Cell<C,BT>* king, Cell<C,BT>* queen, deque<Cell<C,BT>*>& Queue, num rootdim,
                    typename CLIST::iterator& kpos, typename CLIST::iterator& qpos, bool makekgens)
{
	if (GPTALK || CORETALK || AKQTALK)
	{
...
// makes morse complex using coreduce iteratively
// statuses: (the second integer returned value):
// Emil: Arguments order changed !!!!! this was due to fact that default value params shoud be last params
template <typename C, typename BT>
pair<num,bool> MComplex<C,BT>::RedMorsify(const BT& birth, BT except, bool tracer, bool deep)
{
...
num MComplex<C,BT>::MorseReduce(bool savegens, bool tracer)
{
	...
	for(cit = this->clist.rbegin(); cit != this->clist.rend(); ++cit)
	{
		...
		oldsz = this->size();
		numrem = RedMorsify(birth, BANBT,tracer, !savegens); // deep removal if generators are not needed!
